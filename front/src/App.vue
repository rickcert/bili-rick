<template>
  ...
  <div class="use-info">
    <el-collapse :accordion="true" v-model="activeNames">
      ...
      <el-collapse-item v-show="audioUrl != '' && audioUrl != ''" title="音频地址" name="视频地址">
        <p>{{ audioUrl }}</p>
        <p>
          <a :href="audioUrl" target="_blank">自行下载音频</a>
          <a href v-show="downAudioStatus== false" @click.prevent="downAudio">下载音频</a>
          <a href v-show="downAudioStatus == true" @click.prevent="cancelAudioDownload">取消下载</a>
          <el-progress v-show="downAudioStatus == true" :percentage="audioProgress"></el-progress>
        </p>
      </el-collapse-item>
      <el-collapse-item v-show="bgUrl != '' && videoUrl != ''" title="封面地址" name="2">
        <p><a rel="noreferrer" :href="imgUrl" target="_blank">预览图片</a> {{ imgUrl }}
        </p>
        <el-image :src="imgUrl"/>
      </el-collapse-item>
  ...
  <div class="footer"><a href="https://crond.dev" target="_blank" class="copyright"pcked="1">crond.dev</a>
    <span class="copyright">丨</span><a href="https://zhouql.vip"
  ...
  data() {
    return {
      ...
      info: Object.freeze({
        ...
        name: "BiliBili视频下载",
        desc: "bili.crond.dev",
        link: "http://bili.crond.dev"
      }),
      audioLoading: 0,
      audioProgress: 0,
      downAudioStatus: false,
    };
  },
  methods: {
    cancelDownload() {
    ...
    downAudio() {
      ...
      this.$notify({
        title: "下载中",
        message: "音频正在下载中，请等待进度条...",
        iconClass: "el-icon-loading",
        duration: this.audioloading
      });
      source = axios.CancelToken.source();
      this.downAudioStatus = true;
      axios.get(`${this.audioUrl}`, {
        responseType: "blob",
        cancelToken: source.token,
        onDownloadProgress: evt => {
          this.audioProgress = parseInt((evt.loaded / evt.total) * 100);
        }
      }).then(resp => {
        this.audioloading = 5;
        let blobUrl = window.URL.createObjectURL(resp.data);
